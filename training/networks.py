...
class SongUNet(torch.nn.Module):
    def __init__(self,
        ...
        # TODO out_features=noise_channels
        self.map_augment = Linear(in_features=augment_dim, out_features=768, bias=False, **init) if augment_dim else None
        # TODO in_features=noise_channels
        self.map_layer0 = Linear(in_features=768, out_features=emb_channels, **init)
        ...
        # TODO
        self.wresnet = Wide_ResNet()
        self.latent_to_temb = nn.Linear(128, 128*4)

    # TODO
    def forward(self, x0, x, noise_labels, class_labels, augment_labels=None):
        # Mapping.
        emb = self.map_noise(noise_labels)
        emb = emb.reshape(emb.shape[0], 2, -1).flip(1).reshape(*emb.shape) # swap sin/cos

        # TODO
        _, _, z = self.wresnet(x0, emb)
        # batch*768 originally batch*256
        emb = torch.cat((emb, self.latent_to_temb(z)), dim=-1)
...
class VEPrecond(torch.nn.Module):
    ...
    # TODO
    def forward(self, x0, x, sigma, class_labels=None, force_fp32=False, **model_kwargs):
        x = x.to(torch.float32)
        sigma = sigma.to(torch.float32).reshape(-1, 1, 1, 1)
        class_labels = None if self.label_dim == 0 else torch.zeros([1, self.label_dim], device=x.device) if class_labels is None else class_labels.to(torch.float32).reshape(-1, self.label_dim)
        dtype = torch.float16 if (self.use_fp16 and not force_fp32 and x.device.type == 'cuda') else torch.float32
        ...
        # TODO
        F_x, z = self.model((c_in * x0).to(dtype), (c_in * x).to(dtype), c_noise.flatten(), class_labels=class_labels, **model_kwargs)
